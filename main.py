...
def create_bonus():
    ...
    bonus_rect = pygame.Rect(random.randint(250,WIDTH-250),  0, *bonus_size)
...
while playing:
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            playing = False
        if event.type == CREATE_ENEMY:  #create enemies
            enemies.append(create_enemy())
        if event.type == CREATE_BONUS:  #create bonus
...
#Control
    ...
    main_display.blit(player, player_rect)#player on display
